...
## Overview
This dataset contains comprehensive information about Air Quality Index (AQI) measurements across multiple cities worldwide, along with related demographic, weather, and geographic information. The dataset is structured in a star schema design with fact and dimension tables for efficient data analysis.
...
- `S02` (FLOAT) - Sulfur Dioxide concentration (μg/m³)
...
The dataset follows a star schema design with Cities and Date_Dimension as dimension tables, connecting to fact tables (AQI Data and Weather):
...
1. **Countries → Cities** (One-to-Many) `[1:*]`
   ...
   - **Join**: `Countries.country_id = Cities.country_id`
...
2. **Cities → AQI Data** (One-to-Many) `[1:*]`
   ...
   - **Join**: `Cities.city_id = AQI_Data.city_id`
...
3. **Cities → Weather** (One-to-Many) `[1:*]`
   ...
   - **Join**: `Cities.city_id = Weather.city_id`
...
4. **Demographics → Cities** (Many-to-One) `[*:1]`
   ...
   - **Join**: `Demographics.city_id = Cities.city_id`
...
5. **Date_Dimension → AQI Data** (One-to-Many) `[1:*]`
   ...
   - **Join**: `Date_Dimension.date_key = AQI_Data.date_key`
...
6. **Date_Dimension → Weather** (One-to-Many) `[1:*]`
   ...
   - **Join**: `Date_Dimension.date_key = Weather.date_key`
...
7. **AQI Data ↔ Weather** (Parallel Fact Tables, Same Grain)
   ...
   - **Join**: `AQI_Data.city_id = Weather.city_id AND AQI_Data.date_key = Weather.date_key`
